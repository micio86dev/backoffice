 ...
 * candidate id, and the redaction would look like it was working.
 */
export function gaConfigPayload(pagePath: string): GaConfigPayload {
  return {
    ...
    // audience. A remarketing segment built from this traffic would tell every
    // advertiser who buys it which companies are running assessments — a
    // commercial signal that is not BEAI's to sell.
    allow_google_signals: false,
    allow_ad_personalization_signals: false,
