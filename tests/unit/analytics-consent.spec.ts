...
import { ANALYTICS_CONSENT_KEY, readAnalyticsConsent } from '~/utils/analytics-consent'
...
describe('readAnalyticsConsent', () => {
  ...
  it('uses a key that says what it is about', () => {
    // Namespaced and specific. The candidate app uses the same key on its own
    // origin, so the two never collide — an operator's consent and a
    // candidate's are different decisions by different people.
