...
describe('redactFreeText — message and exception values are prose, not addresses', () => {
  it('10. leaves ordinary prose untouched, but cuts a PATH-shaped query', () => {
    // The first half is the original point: `redactUrl` treats its whole
    // argument as an address, so running a console warning through it split the
    // text on its first `#` or `?` and silently dropped the rest.
    ...
    // The second half REVERSES what this test used to assert, deliberately.
    // `/x?y=1` survived here while `redactUrl` cut the identical string — and
    // this module's stated invariant is that two passes claiming the same
    // promise must not disagree on the same input. The magic-link token rides
    // exactly this shape: `/auth/magic?token=<jwt>` reached `transaction`, a
    // breadcrumb and `extra` verbatim.
    //
    // The COST is stated rather than hidden: a harmless `?y=1` goes too. That
    // is the same trade `redactUrl` already makes, for the same reason — an
    // allowlist of safe parameter names is a promise nobody could keep.
    expect(redactFreeText('XHR failed: /x?y=1')).toBe('XHR failed: /x')
...
describe('round 7 — the remaining latent gaps', () => {
  it('takes the rest of the line when a FREE-TEXT query carries a raw space', () => {
    // This test used to assert the opposite — that the tail after a raw space
    // SURVIVED — and documented it as a latent gap. It is closed now, and the
    // reason the trade changed is that `q` is the participants filter: by this
    // module's own header it carries a candidate's name, so ` Lovelace`
    // standing was a leak, not a curiosity.
    //
    // The cut is narrowed to the keys whose value can contain a space at all. A
    // credential never can — a JWT, a signature, an api key are single tokens —
    // so `?token=<jwt> failed` still keeps ` failed` readable.
...
describe('the spreads that carried unhandled fields out', () => {
  ...
  it('SCRUBS a malformed breadcrumbs or exception value, not merely survives it', () => {
    ...
    const badBreadcrumbs = scrubSentryEvent({
      breadcrumbs: {
        0: { message: 'loaded /api/participants/42', data: { candidate_ref: 'CR-99' } },
      },
    } as unknown as ScrubbableEvent)

    const badValues = scrubSentryEvent({
      exception: { values: ['boom /api/participants/42 for ada@acme.test'] },
    } as unknown as ScrubbableEvent)

    const badElement = scrubSentryEvent({
      breadcrumbs: ['loaded /api/participants/42 for ada@acme.test'],
    } as unknown as ScrubbableEvent)

    expect(JSON.stringify(badBreadcrumbs.breadcrumbs)).not.toContain('CR-99')
    expect(JSON.stringify(badBreadcrumbs.breadcrumbs)).not.toContain('/participants/42')
...
describe('one key rule, and an address that ends a sentence', () => {
  it('redacts an address followed by a full stop', () => {
    // The atomic domain run has to hold the TLD inside its lookahead. With the
    // TLD outside, `mario@example.test.` consumed the trailing dot and then had
    // nothing left to match — an address at the end of a sentence stopped being
    // redacted, which is where addresses in prose usually sit.
...
describe('the third half of the plural rule', () => {
  ...
  it('scrubs a non-string transaction and message', () => {
    const scrubbed = scrubSentryEvent({
      transaction: { name: '/api/participants/42' },
...
describe('the fast path and the miss path must answer the same', () => {
  it('redacts what trails the placeholder on a route HIT', () => {
    ...
    const withAddress = redactFreeText('/participants/42/notes/jane@acme.test')
    const withLink = redactFreeText('/participants/42/notes/https://bo.test/interview/FASTTOKEN')

    expect(withAddress).toContain(':id')
  ...
  it('gives contexts.response its URL rules, which is its actual delta', () => {
    ...
    const scrubbed = scrubSentryEvent({
      contexts: { response: { url: 'https://bo.test/participants/42/transcript?q=Ada' } },
    } as unknown as ScrubbableEvent)
...
describe('the URL sink answers like its twin', () => {
  it('cuts a reset credential the anchored rule does not name', () => {
    ...
    expect(redactUrl('/x/reset-password/LIVE-TOKEN')).not.toContain('LIVE-TOKEN')
    expect(redactFreeText('/x/reset-password/LIVE-TOKEN')).not.toContain('LIVE-TOKEN')
  })
  ...
  it('cuts it through a navigation breadcrumb too', () => {
    const scrubbed = scrubBreadcrumb({
      category: 'navigation',
      data: { to: '/x/reset-password/LIVE-TOKEN' },
    } as unknown as Parameters<typeof scrubBreadcrumb>[0])
...
describe('a multi-word denied key behind a prefix', () => {
  ...
  it('runs a frame URL through `redactUrl`, not only the embedded-path pass', () => {
    // `redactEmbeddedPaths` matches an id with a class that deliberately
    // excludes `,` `;` `:`; `redactAnalyticsPath` — reachable only through
    // `redactUrl` — uses `[^/]+` and eats the whole segment. Without the URL
    // pass on the frames, `:id,Ada-Lovelace` came back with the surname
    // standing NEXT TO a placeholder that reads as if redaction succeeded.
    const err = new Error('boom')

    err.stack = 'Error: boom\n    at s (https://bo.test/participants/42,Ada-Lovelace/x.js:1:1)'
    ...
    expect(stack).not.toContain('Ada-Lovelace')
...
describe('the miss that inequality could not detect', () => {
  it.each([
    ['a query string', '/x/reset-password/LIVE-TOKEN?email=a@b.test'],
    ['a trailing slash', '/x/reset-password/LIVE-TOKEN/'],
