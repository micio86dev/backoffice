...
- **`typecheck` does NOT cover `tests/unit/**`.** `.nuxt/tsconfig.app.json` includes
  `../tests/nuxt/**`, never `tests/unit/**` — proven: `const x: number = 'str'` exits 0 under
  `tests/unit/`, exits 2 under `tests/nuxt/`. Any **type-level** assertion (contract tests,
  `expectTypeOf`) placed in `tests/unit/` is a gate that can never fail. Put it in `tests/nuxt/`.
- **A test that stubs the thing it is verifying proves nothing.** A unit test stubbing
  `useLocaleHead` passed against an `<html lang>` binding that was genuinely broken; only the
  container E2E caught it. When a fix depends on framework behaviour, assert the framework's
  actual precondition too.
