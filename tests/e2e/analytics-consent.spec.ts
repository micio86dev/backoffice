 ...
 * request leaves before consent, and that the banner stays away from the pages
 * showing candidate data.
 *
 ...
 * ordering — asking somebody to make a privacy choice before they have even got
 * through the door is asking a stranger.
 */
const SAFE_PAGE = '/health'
...
test.describe('Analytics consent', () => {
  ...
  test('it asks on an ordinary page', async ({ page }) => {
  ...
  test('it does NOT appear on login, where credentials are typed', async ({ page }) => {
    ...
    // Analytics never runs here, so asking would be asking about nothing — and
    // a cookie dialog floating over a credential form is the wrong thing in the
    // wrong place.
    await expect(page.getByTestId('analytics-consent')).toBeHidden()
  })
